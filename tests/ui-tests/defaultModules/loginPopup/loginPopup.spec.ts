...
test.describe('Login popup positive and negative checks', () => {
  ...
  test.use({ storageState: './helpers/cookieState/cookies.json' });
  ...
  test('Verify user can NOT login his account with invalid password and valid email', async () => {
    ...
    expect(await locators.loginPopup.incorrectFieldError.innerText()).toContain(
  ...
  test('Verify the login popup fields have empty values validation', async () => {
    ...
    expect(await locators.loginPopup.incorrectFieldError.first().innerText()).toContain(
      errorMessages.emailNotEntered
    );
    expect(await locators.loginPopup.incorrectFieldError.last().innerText()).toContain(
      errorMessages.passwordNotEntered
    );
  });
});
