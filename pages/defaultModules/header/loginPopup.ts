...
export default class LoginPopup {
  ...
  constructor(page: Page) {
    ...
    this.accountBtnDropDown = page.locator('[data-testid="userToolsDropDown"]');
    ...
    this.registrationLink = page
      .locator('[data-testid="loginForm"]')
      .getByRole('button', { name: 'Регистрация' });
    this.basketCounter = page.locator('[data-testid="header-count"]');
  }

  public async triggerTheLoginPopup() {
    //Click on the account button and then on the 'Войти'-button
  ...
  public async logOutTheAccount() {
    //Logout the test account
    await this.accountBtn.click();
    expect(await this.accountBtnDropDown.isVisible()).toBe(true);
